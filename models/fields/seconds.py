...
class Seconds(object):
    ...
    def delay(self, dt: datetime) -> timedelta:
        assuming_today = dt - (
            dt.replace(hour=0, minute=0, second=0, microsecond=0) + self.to_timedelta()
        )
        return min(
            assuming_today,
            assuming_today + timedelta(days=1),
            assuming_today - timedelta(days=1),
            key=abs,
        )
    ...
    def __add__(self, other: timedelta):
        new_seconds = self.seconds + int(other.total_seconds())
        while new_seconds < 0:
            new_seconds += 24 * 60 * 60

        return Seconds(new_seconds)
