...
from multigtfs.models import Block, Feed, Route, Service, Shape, Stop, StopTime, Trip


class TripTest(TestCase):
    def setUp(self):
        self.feed = Feed.objects.create()
        self.route = Route.objects.create(feed=self.feed, route_id="R1", rtype=3)
...
""",
    ...
    def test_update_geometry_has_stoptimes(self):
        stop1 = Stop.objects.create(
            feed=self.feed, stop_id="STAGECOACH", point="POINT(-117.133162 36.425288)"
        )
        stop2 = Stop.objects.create(
            feed=self.feed, stop_id="TAVERN", point="POINT(-117.14 36.43)"
        )
        trip = Trip.objects.create(route=self.route, trip_id="T1")
        ...
        trip.update_geometry()
        self.assertEqual(
            trip.geometry.coords, ((-117.133162, 36.425288), (-117.14, 36.43))
        )
